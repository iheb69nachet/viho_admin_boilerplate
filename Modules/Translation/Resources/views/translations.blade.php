...
@section('title')Translations for Language {{ $language->name }} @endsection
            ...
            <div class="col-sm-12">
                <div class="card">
                    <div class="card-header">
                        <h5>Translations for {{ $language->name }}</h5>
                    </div>
                    <div class="card-body">
                        ...
                        <table id="translationsTable" class="table">
                            ...
                            <tbody id="translationsTableBody">
                                ...
                                @foreach($translations as $translation)
                                    <tr data-key="{{ $translation->key }}">
                                        <td><input type="text" class="form-control" name="parent_key" value="{{ $translation->parent_key }}" disabled></td>
                                        <td><input type="text" class="form-control" name="key" value="{{ $translation->key }}" disabled></td>
                                        <td><input type="text" class="form-control" name="value" value="{{ $translation->value }}"></td>
                                        <td>
                                            <button class="btn btn-primary btn-save" data-key="{{ $translation->key }}" data-language="{{ $language->id }}">Save</button>
                                        </td>
                                    </tr>
                                @endforeach
                            </tbody>
                        </table>
                        <a href="{{ route('translations.create', ['language' => $language->id]) }}" class="btn btn-success">Add Translation</a>
                    </div>
                </div>
            </div>
        </div>
